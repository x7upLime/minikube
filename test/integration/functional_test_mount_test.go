...
func validateMountCmd(ctx context.Context, t *testing.T, profile string) { // nolint
	...
	t.Run("verify-cleanup", func(t *testing.T) {
		...
		defer func() {
			t.Logf("\n\nTrying to kill what already could've been killed\n\n")
		...
		checkMount := func() error {
			...
			for _, mnt := range guestMountPaths {
				...
				t.Logf("\t\t✔️ Found... Returning nil err\n")
			}
			return nil
		}
		if err := retry.Expo(checkMount, time.Millisecond*500, Seconds(15)); err != nil {
		...
		checkProcsAlive := func(end chan bool) {
			t.Logf("\n\nChecking if processes are still alive after killing signal\n\n")
		...
		t.Logf("\n\nKilling all mount processes owned by minikube\n\n")
		rr, err := Run(t, exec.Command(Target(), "mount", "-p", profile, "--kill=true"))
		t.Logf("\nCmd STDOUT:\n%s\n", rr.Stdout)
		if err != nil {
			t.Fatalf("\n\nfailed while trying to kill mounts\n\n")
		}
